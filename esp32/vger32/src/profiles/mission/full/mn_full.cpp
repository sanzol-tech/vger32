 ...
 * mn_full.cpp
 *
 * Full mission logic: LED state tracking, periodic MQTT sensor publish,
 * and mission-specific command handling.
 */
...
void mn_full_on_cmd(const char *topic, const char *payload) {
    ...
    if (strcmp(cmd, "publish_now") == 0) {
        ...
        sys_log(LOG_INFO, "Full", "publish_now flagged");
        return;
    }
    ...
    sys_log(LOG_DEBUG, "Full", "cmd ignored — %s", cmd);
}
